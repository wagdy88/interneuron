...
def main():
								...
								for gkbar_iahp in gkbar_iahp_values:
									for gcabar_it2INT in gcabar_it2INT_values:
										for pcabar_icalINT in pcabar_icalINT_values:
											...
											param_group = group.create_group(f"combo_{index:05d}")
											for key, value in param_set.items():
												param_group.attrs[str(key)] = value
											index += 1
	...
	# Open an HDF5 file for saving results
		with h5py.File("simulation_results.h5", "w", driver="mpio", comm=MPI.COMM_WORLD) as h5file:
	...
	# Configure NEURON model
				...
				sim_group.create_dataset("time", data=time_vector, chunks=True, compression="gzip")
				sim_group.create_dataset("voltage", data=voltage_vector, chunks=True, compression="gzip")
	comm.Barrier()
